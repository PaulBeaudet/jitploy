            ...
            if(req.body){
                ...
                mongo.db[RELAY_DB].collection('github_secrets').findOne(findQuery, mongo.bestCase(function onFind(doc){
                    if(github.verifyHook(req.headers['x-hub-signature'], req.body, doc.secret)){
                        console.log('deployment event for ' + req.body.repository.name);
                        socket.deploy(req.body.repository.ssh_url.split('//')[1]);   // given service registered only by git@github url
                        socket.deploy(req.body.repository.clone_url.split('//')[1]); // given service registered only by https url
